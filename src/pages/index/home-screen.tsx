...
const svgData = (body: string, width: number, height: number) => `data:image/svg+xml;utf8,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" fill="none">${body}</svg>`)}`
const HOME_SVG_SURFACE_TODAY = svgData('<rect width="339" height="148" rx="20" fill="#FFF0C9"/>', 339, 148)
const HOME_SVG_SURFACE_QUICK = svgData('<rect width="163" height="112" rx="22" fill="#FFFFFF"/>', 163, 112)
const HOME_SVG_SURFACE_RECENT = svgData('<rect width="339" height="67" rx="18" fill="#FFFFFF"/>', 339, 67)
const HOME_SVG_DETAIL_BG = svgData('<rect width="76" height="42" rx="15" fill="#F4B43A"/>', 76, 42)
const QUICK_ICON_START_VECTOR = svgData('<path d="M12 2V22M2 12H22" stroke="#B98308" stroke-width="1.6" stroke-linecap="round"/>', 24, 24)
...
function QuickIcon({ type }: { type: 'start' | 'continue' }) {
  return <Image className={`home-hf-quick-icon ${type}`} src={type === 'start' ? QUICK_ICON_START_VECTOR : QUICK_ICON_CONTINUE} mode='aspectFit' />
}
