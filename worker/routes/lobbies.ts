...
const LOBBY_SEATS = [0, 1, 2, 3] as const

function lobbySeat(value: unknown): number | null {
  return typeof value === 'number' && Number.isInteger(value) && value >= 0 && value < LOBBY_CAPACITY ? value : null
}

function firstFreeSeat(lobby: MatchLobby): number | null {
  const occupied = new Set(lobby.members.map(member => member.seat).filter((seat): seat is number => seat !== null))
  return LOBBY_SEATS.find(seat => !occupied.has(seat)) ?? null
}
...
async function loadLobby(db: D1Database, idOrCode: string, viewerUserId: string): Promise<MatchLobby | null> {
  ...
  const membersResult = await db.prepare(`
    SELECT lm.id, lm.user_id, lm.friend_id, lm.name, lm.avatar_seed, lm.seat, lm.joined_at,
    ...
    WHERE lm.lobby_id = ?
    ORDER BY CASE WHEN lm.seat IS NULL THEN 4 ELSE lm.seat END ASC, lm.joined_at ASC, lm.id ASC
...
export function registerLobbyRoutes(app: Hono<Env>) {
  ...
  app.post('/api/match-lobbies/:id/join', async c => {
    ...
    const requestedSeat = lobbySeat(body?.seat)
    const linkedPlaceholder = await c.env.DB.prepare(`
      SELECT lm.id, lm.friend_id, lm.seat
      FROM match_lobby_members lm
      JOIN friends f ON f.id = lm.friend_id
      WHERE lm.lobby_id = ? AND f.user_id = ? AND f.linked_user_id = ?
      LIMIT 1
    `).bind(lobby.id, lobby.ownerUserId, user.id).first<{ id: string; friend_id: string; seat: number | null }>()
    const displayName = user.display_name?.trim() || user.username
    const joinedAt = now()
    if (linkedPlaceholder) {
      const seat = lobbySeat(linkedPlaceholder.seat) ?? requestedSeat ?? firstFreeSeat(lobby)
      if (seat === null) return jsonError(c, '这桌已经满员', 409)
      const occupied = lobby.members.find(member => member.seat === seat && member.id !== linkedPlaceholder.id)
      if (occupied) return jsonError(c, '这个方位刚被占了，请选择其他位置', 409)
      await c.env.DB.prepare(`
        UPDATE match_lobby_members SET user_id = ?, name = ?, avatar_seed = ?, seat = ?, updated_at = ? WHERE id = ?
      `).bind(user.id, displayName, avatarSeed(displayName), seat, joinedAt, linkedPlaceholder.id).run()
    } else {
      if (lobby.members.length >= LOBBY_CAPACITY) return jsonError(c, '这桌已经满员', 409)
      const seat = requestedSeat ?? firstFreeSeat(lobby)
      if (seat === null) return jsonError(c, '这桌已经满员', 409)
      if (lobby.members.some(member => member.seat === seat)) return jsonError(c, '这个方位刚被占了，请选择其他位置', 409)
      await c.env.DB.prepare(`
        INSERT INTO match_lobby_members(id, lobby_id, user_id, friend_id, name, avatar_seed, seat, joined_at, updated_at)
        VALUES(?, ?, ?, NULL, ?, ?, ?, ?, ?)
      `).bind(uid(), lobby.id, user.id, displayName, avatarSeed(displayName), seat, joinedAt, joinedAt).run()
    }
  ...
  app.post('/api/match-lobbies/:id/members', async c => {
    ...
    const body = await c.req.json().catch(() => null) as { source?: unknown; friendId?: unknown; name?: unknown; seat?: unknown } | null
    const source = body?.source === 'self' || body?.source === 'friend' || body?.source === 'guest' ? body.source : null
    if (!source) return jsonError(c, '添加方式无效')
    const seat = lobbySeat(body?.seat) ?? firstFreeSeat(lobby)
    if (seat === null) return jsonError(c, '四人已齐', 409)
    const at = now()

    if (source === 'self') {
      if (lobby.members.some(member => member.userId === user.id)) return jsonError(c, '你已经在牌桌上', 409)
      if (lobby.members.some(member => member.seat === seat)) return jsonError(c, '这个方位已经有人了', 409)
      const displayName = user.display_name?.trim() || user.username
      await c.env.DB.prepare(`
        INSERT INTO match_lobby_members(id, lobby_id, user_id, friend_id, name, avatar_seed, seat, joined_at, updated_at)
        VALUES(?, ?, ?, NULL, ?, ?, ?, ?, ?)
      `).bind(uid(), lobby.id, user.id, displayName, avatarSeed(displayName), seat, at, at).run()
    } else {
      ...
      const occupied = lobby.members.find(member => member.seat === seat)
      if (occupied && occupied.id !== linkedMember?.id) return jsonError(c, '这个方位已经有人了', 409)
      if (linkedMember) {
        await c.env.DB.prepare('UPDATE match_lobby_members SET friend_id = ?, seat = ?, updated_at = ? WHERE id = ?')
          .bind(friend.id, seat, at, linkedMember.id).run()
      } else {
        await c.env.DB.prepare(`
          INSERT INTO match_lobby_members(id, lobby_id, user_id, friend_id, name, avatar_seed, seat, joined_at, updated_at)
          VALUES(?, ?, NULL, ?, ?, ?, ?, ?, ?)
        `).bind(uid(), lobby.id, friend.id, friend.name, Number(friend.avatar_seed), seat, at, at).run()
      }
    }
  ...
  app.post('/api/match-lobbies/:id/start', async c => {
    ...
    const ordered = [...lobby.members].sort((left, right) => (left.seat ?? 99) - (right.seat ?? 99))
    if (ordered.some((member, index) => member.seat !== index)) {
      return jsonError(c, '请先为东、南、西、北各安排一位玩家')
    }
